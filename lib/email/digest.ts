...
export type Scope = {
  scope_type: 'all' | 'company' | 'property'
...
export async function sendSubscription(sub: Subscription, opts?: { test?: boolean; ref?: Date }): Promise<{ ok: boolean; reason?: string }> {
  ...
  const freq = opts?.test ? 'daily' : sub.frequency
  const data = await getDigestData(sub, freq, opts?.ref)
  const subject = `Sol Eterno · Movimientos ${data.periodWord} (${data.scopeText}) — ${data.checkins.length} in / ${data.checkouts.length} out`

  const { error } = await r.emails.send({ from: FROM, to: [sub.email], subject, html: renderDigestHtml(data) })
